                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" width="100%" cellspacing="0">
                            ...
                            <tr>
                                ...
                                <td>{{ $transaksi->tanggal_transaksi instanceof \Carbon\Carbon ? $transaksi->tanggal_transaksi->format('d F Y') : \Carbon\Carbon::parse($transaksi->tanggal_transaksi)->format('d F Y') }}</td>
                            </tr>
                            <tr>
                                <th>Estimasi Selesai</th>
                                <td>{{ $transaksi->tanggal_selesai ? ($transaksi->tanggal_selesai instanceof \Carbon\Carbon ? $transaksi->tanggal_selesai->format('d F Y') : \Carbon\Carbon::parse($transaksi->tanggal_selesai)->format('d F Y')) : '-' }}</td>
                            </tr>
                            <tr>
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <!-- Nama route diperbarui sesuai prefix karyawan -->
        <form action="{{ route('karyawan.transaksi.updateStatus', $transaksi->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label for="status_transaksi" class="form-label font-weight-bold">Status Transaksi</label>
